...
- 长期分支名称只允许 `main` 和已经明确批准的 `release/v<major>.<minor>`；例如 `release/v3.7`。长期分支不添加日期，也不得用生命周期、成员姓名或单个功能命名。
...
- 用户未指定基线时，新任务分支默认从最新 `origin/main` 创建；针对历史冻结版本的修复才从对应 `origin/release/v<major>.<minor>` 创建。不得默认从陈旧的本地 `main`、已合并分支或 `wip/*` 分支继续派生正式交付分支。
...
- 首次推送任务分支统一使用 `git push -u origin <branch-name>` 建立同名 upstream。禁止把任务分支推送到另一个远程分支名，也禁止默认直接向 `main` 或 `release/*` 推送。
