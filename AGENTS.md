...
- 长期硬规则见 [docs/specialized/蛋宝宝_前端开发与AI协作规则_v1.1.md](docs/specialized/蛋宝宝_前端开发与AI协作规则_v1.1.md)；v1.0 只作历史文档。
- 产品只维护开发状态与正式状态：`develop` = 开发状态，`trial` 与 `release` = 正式状态，trial 不维护第三套逻辑。正式用户功能在三个环境中一致可用；只有 fixture 切换、测试按钮、状态切换器、开发说明、独立开发验收入口、调试日志与测试参数限 `develop`。正式功能开关与开发调试开关必须分开维护。
- 从用户真实入口开发和验收；用户确认的可见内容是封闭清单，AI 不得擅自增加文案、装饰、入口或层级。
- 自动测试不能代替 COO 人工视觉验收；用户没要求时不使用 computer-use。
- 前端只做静态 UI、本地内容和接口适配层；本地状态不等于服务端能力。
- 正式代码只在 `/Users/octopusgump/dev/eggbabe-main` 维护；长期分支只保留 `main`；任务分支验收后合入、归档并清理。
