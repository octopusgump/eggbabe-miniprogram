...
- **产品口径**：三屏连续空间；每 5 小时一个稳定状态；仅家、旅行、打工、上学四个大场景；正式旅行目的地为大理、北京、西双版纳、东京；居家固定动作、条件对话，外出只写信并显示空房；旅行画面只在回家后送达的独立明信片中展示；行动后才产生纪念物。
...
- **验证**：`post-hatch-companion.test.js`、`post-hatch-panel-scenes.test.js`、`demo-pages.test.js`、`memory-demo-preview.test.js`、`release-gate.test.js`、`scene-window-geometry.test.js`、`magic-window-tokyo.test.js`、`verify-package-assets.js`。
- **当前素材进度**：玉兔卧室 6 个居家动作已有晴天日间 / 夜间完整全景；东京开发预览有 12 张候选明信片和独立旅途回放页。
