...
| M4 破壳与收藏卡 | 校验破壳门槛，生成一蛋一份收藏卡并保存分享 | `hatch / collection-card / h5-card` → `birth-card-h5 / cloud-api / pet-store` | **live 待闭环** | 后端固定结果、正式 H5 域名、CDN素材和相册真机链路 |
| M5 破壳后陪伴 | 查看三屏生活空间、完成固定动作、对话、写信和回忆 | `home / life-scene / life-scenes` → `post-hatch-companion / life-scenes / chat-service` | **部分落地** | 正式后端四接口、角色与场景素材、明信片和魔法窗素材 |
...
- **产品口径**：达到 100 后仍需昵称、出生礼物和孵化回顾；收藏卡一蛋一份，H5 不生成随机名字、插画或业务时间。
...
- **产品口径**：三屏连续空间；每 5 小时一个稳定状态；仅家、旅行、打工、上学四个大场景；居家固定动作、条件对话，外出只写信；行动后才产生纪念物。
- **用户入口**：`pages/home`、`pages/life-scene`、`pages/life-scenes`。
- **核心服务**：`services/post-hatch-companion.js`、`utils/life-scenes.js`、`services/environment-state.js`、`services/chat-service.js`、`utils/memory-demo-preview.js`。
- **live 接口**：`getPostHatchHome`、`performPostHatchAction`、`sendPostHatchLetter`、`getPostHatchMemories`，见 `services/cloud-api.js:105-115`。
- **验证**：`post-hatch-companion.test.js`、`post-hatch-panel-scenes.test.js`、`demo-pages.test.js`、`memory-demo-preview.test.js`、`scene-window-geometry.test.js`、`magic-window-tokyo.test.js`。
- **未闭环条件**：正式角色姿态、左右屏状态素材、明信片、心情头像和三张魔法窗景区图未齐；魔法窗入口应继续关闭；右侧布置区本轮明确不实施拖放和装饰存储。
