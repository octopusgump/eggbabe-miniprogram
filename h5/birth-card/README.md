...
卡面采用三段式 9:16 模板：顶部为名字标题，中部为服务端已选定的 4:5 水彩插画，底部为 MBTI / 星座 / 生日三行信息。插画使用 cover 方式铺满视口，卡面不使用黑色内描边。名字按需加载 Google Fonts OFL 版 `ZCOOL KuaiLe` 的 `text=` 子集，失败时回退 PingFang SC。
