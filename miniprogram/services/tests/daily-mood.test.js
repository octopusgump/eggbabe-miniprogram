...
assert.equal(componentStyles.includes('.pet-mood-tab__name{display:block;max-width:100%') && componentStyles.includes('.pet-mood-tab__sentence{width:0;max-height:0') && componentStyles.includes('.pet-mood-tab--expanded .pet-mood-tab__sentence{width:auto;max-width:100%}') && componentStyles.includes('.pet-mood-tab__sentence--visible{max-height:60rpx'), true, '收起态隐藏句子不得撑宽 Tab，展开态仍必须完整呈现句子');
