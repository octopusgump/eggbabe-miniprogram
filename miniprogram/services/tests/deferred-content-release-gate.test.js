...
['pages/life-scenes/life-scenes.js', 'pages/journey-scene/journey-scene.js'].forEach(relative => {
  assert.equal(read(relative).includes('guardDeferredContent()'), true, `${relative} 必须阻止正式版旧路径绕过入口`);
});
['pages/album/album.js', 'pages/collection-card/collection-card.js', 'pages/h5-card/h5-card.js'].forEach(relative => {
  assert.equal(read(relative).includes('releaseSurface'), false, `${relative} 已正式开放，不得再受延后内容门禁限制`);
});
assert.equal(read('pages/my/my.wxml').includes('我的收藏卡'), true, '我的页必须展示正式收藏卡入口');
assert.equal(read('pages/my/my.js').includes("onNavAlbum() { wx.navigateTo({ url: '/pages/album/album' }); }"), true, '我的页收藏卡入口必须进入现有收藏卡列表');
...
console.log('收藏卡正式开放，其他延后复杂内容继续受门禁限制校验通过。');
