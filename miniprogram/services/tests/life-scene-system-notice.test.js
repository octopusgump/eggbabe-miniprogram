...
assert.equal(logic.includes("this.showSystemNotice(chatAccess && chatAccess.message || '聊天权限正在同步，请稍后再试。', 'warning')"), true, '聊天不可用必须是 warning 系统提示');
...
assert.equal(logic.includes("this.showFeedback(chatAccess && chatAccess.message || '聊天权限正在同步，请稍后再试。')"), false, '聊天不可用不得再借角色对白表达');
