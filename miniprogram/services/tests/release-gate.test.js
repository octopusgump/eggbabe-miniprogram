...
const context = Object.assign({}, global.page, {
  data: Object.assign({}, global.page.data, { agreed: true, authorizing: false }),
  setData(patch) {
...
assert.equal(context.data.systemNoticeText, '账号服务尚未接入，请稍后再试', '正式后端未配置时必须继续阻断假成功');
assert.equal(toast, '', '页面可承接的账户服务失败不得重复显示系统 Toast');
