...
const AWAY_STATES = [
  ...
  { key: 'beijing', major: 'travel', majorLabel: '旅行', label: '北京', line: '银杏叶落在红墙边，风一吹就翻了个金色的面。', keepsake: { id: 'ginkgo-leaf', name: '一片银杏叶', story: '它落下来时正好停在我的鞋尖。我把它夹在纸页里带回家，边缘慢慢变得像金色的小扇子。每次拿出来，我都会先看看它有没有又轻一点。' } },
...
const STORY_LINE = [
  ...
  HOME_STATES[7], AWAY_STATES[4], HOME_STATES[7], HOME_STATES[0], AWAY_STATES[5], HOME_STATES[3],
...
function stateForSlot(slotIndex, slotStart) {
  ...
  const state = isNightHour(hour) && base.major !== 'home'
    ? (hour < 4 ? HOME_STATES[0] : HOME_STATES[5])
    : base;
...
function resolveDefinition(major, key) {
  ...
  return Object.assign({}, state, {
    ...
    canTalk: atHome,
    action: atHome ? Object.assign({}, state.action) : null,
