...
(async () => {
  ...
  assert.equal(appConfig.pages.includes('pages/chat-records/chat-records'), true, '我的页必须提供独立对话记录页面');
  assert.equal(appConfig.pages.includes('pages/account/account') || appConfig.pages.includes('pages/deregister/deregister'), false, '线上范围不得恢复旧账号或注销页面');
