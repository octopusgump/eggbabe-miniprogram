...
(async () => {
  ...
  assert.equal(home.includes("wx:if=\"{{stage === 'hatched' && postHatchSnapshot}}\" class=\"state-card\""), true, '生活状态卡只能在破壳后且快照加载成功后展示');
  ...
  assert.equal(home.includes('companion-primary-dock') && home.includes("item.key !== 'draw'") && home.includes("item.key === 'draw'"), true, '许愿池与早教班必须成组显示，画画保持独立入口');
  assert.equal(home.includes('draw-icon-button'), true, '画画必须始终使用紧凑绘图图标');
  assert.equal(homeLogic.includes('再陪我一天') || homeLogic.includes('明天再来解锁'), false, '未开放入口不得使用连续到访或催促文案');
  assert.equal(homeLogic.includes('早教班还在准备中。'), true, '未开放早教班必须使用中性反馈');
  ...
  assert.equal(home.includes('class="eyebrow"') && home.includes('<mood-badge') && home.includes('class="progress-ring"') === false, true, '破壳后首页必须展示角色与每日心情，且不得恢复百分比圆环');
