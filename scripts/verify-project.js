...
if (!lifeSceneTemplate.includes('wx:if="{{currentState && currentState.atHome}}" class="scene-context-entry"') || lifeSceneTemplate.includes('class="away-status-') || lifeSceneTemplate.includes('外出中') || !lifeSceneTemplate.includes('class="scene-action-dock"') || !lifeSceneTemplate.includes('contextActionIcon') || !lifeSceneTemplate.includes('mySettingsIcon') || !lifeSceneTemplate.includes('aria-label="打开我的和设置"') || !lifeSceneLogic.includes('/pages/chat/chat?state_key=') || !lifeSceneLogic.includes("chatAccess.status !== 'available'") || !lifeSceneLogic.includes('scene_chat_button') || !lifeSceneLogic.includes('statusBubbleFor')) errors.push('破壳后居家必须显示左下陪伴入口，外出时左下留空，右下我的/设置保持可用');
